...
def index_db_manager(monkeypatch, mock_settings):
    monkeypatch.setattr(
        "rag_app.db.index_db.get_settings",
        lambda: mock_settings,
    )

    mock_tracker = MagicMock()
    monkeypatch.setattr(
        "rag_app.db.index_db.DBOperationTracker",
        lambda: mock_tracker,
    )
...
async def test_store_index_with_valid_data(
    ...
    with (
        patch(
            "rag_app.db.index_db.psycopg.AsyncConnection.connect",
            new=AsyncMock(return_value=mock_conn),
        ),
        patch(
            "rag_app.db.index_db.register_vector_async",
            new=AsyncMock(),
        ),
    ):
