...
def index_service_manager(monkeypatch, mock_dependencies):
    with (
        patch(
            "rag_app.services.index_services.DataProcessor",
            return_value=mock_dependencies["data_processor"],
        ),
        patch(
            "rag_app.services.index_services.DataManager",
            return_value=mock_dependencies["data_manager"],
        ),
        patch(
            "rag_app.services.index_services.ChunkingManager",
            return_value=mock_dependencies["chunking_manager"],
        ),
        patch(
            "rag_app.services.index_services.EmbeddingManager",
            return_value=mock_dependencies["embedding_manager"],
        ),
        patch(
            "rag_app.services.index_services.IndexDBManager",
