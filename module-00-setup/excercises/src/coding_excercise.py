...
def genrate_gitignore_content(project_type: list[str]):
    ...
    return "Created .gitignore file"


response = genrate_gitignore_content(["python", "java"])
print("Task 3 Output")
print(response)
print("\n", "=" * 60, "\n")
        ...
        def wrapper(*args, **kwargs):
            ...
            while attempts < max_attempts:
                try:
                    ...
                    attempts += 1
                    print(
                        f"Attempt {attempts} failed with error: {e}. Retrying in {backoff**attempts} seconds..."
                    )
...
def call_api(api_client, model_name: str, input_text: str):
    ...
    try:
        if isinstance(api_client, OpenAI):
            response = api_client.responses.create(model=model_name, input=input_text)
...
# result = call_api(openai_client, "gpt-4.1-mini", "Hello World!") # use when OpeanAI API
# print(result)
print("\n", "=" * 60, "\n")
...
check_redis_port = check_open_port(REDIS_HOST, REDIS_PORT)
print("Task 8 Output")
print(f"Redis port status :- {'Open' if check_redis_port else 'Close'}")
print("\n", "=" * 60, "\n")
...
def call_api_check(api_client, model_name: str, input_text: str):
    """
    Call the specified API client with the given model name and input text.
